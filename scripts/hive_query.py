...
def main():
    ...
    # 1. 认证
    print("获取 token ...")
    token = get_token(auth_url, args.username, args.password)
    print("认证成功")

    # 2. 提交查询
    print(f"提交查询 (code={args.code}) ...")
    query_id = submit_query(svc_url, token, args.code, params)
    print(f"queryId: {query_id}")

    # 3. 轮询状态
    print(f"等待查询完成 (轮询间隔 {args.poll_interval}s, 超时 {args.timeout}s) ...")
    elapsed = 0
    hdfs_path = ""
    while elapsed < args.timeout:
        status = get_status(svc_url, token, args.code, query_id)
        state = status.get("state")
        progress = status.get("progress", 0)
        print(f"  状态: {state}, 进度: {progress:.0%}")
        if state == "SUCCESS":
        ...
        raise TimeoutError(f"查询超时 ({args.timeout}s)")

    print(f"查询完成, hdfsPath: {hdfs_path}")

    # 4. 下载文件
    if not hdfs_path:
        print("结果存储在 OBS，请到 OBS 获取数据。")
        return

    print(f"获取文件列表: {hdfs_path}")
    files = list_files(svc_url, token, args.code, hdfs_path)
    ...
    print(f"下载 {len(download_tasks)} 个文件到 {args.output_dir}")
    for f in download_tasks:
        ...
        download_file(svc_url, token, args.code, remote, local)
