...
会话 ID 由后端确定性生成：`session_id = {guide_num}_{union_id}`（导购工号 + 会员标识）。

```
首次生成
  → 后端返回 { session_id, pitch, ... }
  → 前端在页面顶部展示 session_id（不落 localStorage）
  → 同导购 + 同会员再次请求自动复用同一 thread
  → 后端通过 LangGraph thread_id 恢复对话历史
  → Agent 基于历史上下文生成连贯的话术调整（如"换一换"）
...
| 区块 | 说明 |
|---|---|
| 导购信息 | 导购工号（必填，置顶区块；持久化到浏览器本地存储，自动回填） |
| 顾客信息 | 会员标识（必填），称呼、性别、年龄、会员等级、积分（选填） |
| 商品信息 | 支持 1-10 个商品，必填商品名称，可选颜色；多件时第一件为主款 |
| 促销与优惠 | 门店活动（promo_id + 文案，最多 10 条，话术仅引用文案原文）；顾客优惠券（逗号/换行分隔，最多 20 张；勾选「暂无可用券」触发「会员专属优惠」兜底表述） |
| 话术要求 | 风格（邀约工作台四档语气：亲切自然/活力潮流/专业尊贵/简约高效，另有兼容预设）、字数上限、补充要求 |
| 对话记录 | 每次请求以气泡方式展示请求摘要（含风格/活动/券）+ AI 话术，支持一键复制 |
| 会话控制 | 顶部显示当前 session_id（由导购工号 + 会员标识确定性生成）；更换标识即开启新会话 |
